...
def is_cookeding_criteria_satisfied(time, temperature, 
    ...
    return is_well_done(time, temperature, pressure, desired_state) or is_medium(time, temperature, pressure, desired_state)

def is_well_done(time, temperature, pressure, desired_state):    
    return desired_state == 'well-done' and get_cooking_progress(time, temperature, pressure) >= WELL_DONE

def is_medium(time, temperature, pressure, desired_state):
    return desired_state == 'medium' and get_cooking_progress(time, temperature, pressure) >= MEDIUM
